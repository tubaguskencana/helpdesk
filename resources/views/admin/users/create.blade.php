...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="pb-4 border-b border-slate-200">
        ...
        <h1 class="text-xl font-bold text-slate-900 tracking-tight">Add New User</h1>
        <p class="text-sm text-slate-500 mt-0.5">Register a new employee, agent, or administrator.</p>
    </div>
        ...
        <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="role" required class="w-full px-3 py-2 text-sm rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                        <option value="user" {{ old('role') === 'user' ? 'selected' : '' }}>Requester (User)</option>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="job_title" value="{{ old('job_title') }}" placeholder="e.g. IT Specialist"
                           class="w-full px-3 py-2 text-sm rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                </div>
            </div>
            ...
            <div class="pt-2">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-4 h-4 rounded text-blue-600">
            ...
            <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold shadow-sm transition">Create User</button>
            </div>
        </form>
    </div>
</div>
